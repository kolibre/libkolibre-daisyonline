...
int main(int argc, char **argv)
{
    ...
    DaisyOnlineHandler DOHandler(std::string(argv[1]), "faultinvalidparameter/1.2.3");
    ...
    assert(DOHandler.getStatus() == DaisyOnlineHandler::FAULT_INVALIDPARAMETER);
    assert(DOHandler.getLastSoapFaultReason() == "Invalid Parameter");
    ...
    // returnContent
    assert(false == DOHandler.returnContent("not_a_valid_list_id"));
    assert(!DOHandler.good());
    assert(DOHandler.getStatus() == DaisyOnlineHandler::FAULT_INVALIDPARAMETER);
    assert(DOHandler.getLastSoapFaultReason() == "Invalid Parameter");
